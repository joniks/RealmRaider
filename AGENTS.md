...
- Treat test time and agent context as scarce: focused checks expose a concrete risk; one final EditMode and one final PlayMode run prove the frozen candidate. Unity GUI remains the default while the Editor is open. Architect may use the repository CLI wrapper only across an announced, user-approved closed-Editor boundary with Unity Hub open and signed in for the Personal licence; never run two Unity processes against the same project. After every accepted Core commit, run the Android CLI export gate before returning Unity to the user.
