...
- Treat test time and agent context as scarce: focused checks expose a concrete risk; one final EditMode and one final PlayMode run prove the frozen candidate. Unity GUI remains the default while the Editor is open. After an accepted Core commit, export Android through the existing Unity menu without closing or restarting a healthy Editor. Architect may use the repository CLI wrapper only when the Editor is already closed and the user approved that boundary; Unity Hub must remain open and signed in for the Personal licence. Never run two Unity processes against the same project.
