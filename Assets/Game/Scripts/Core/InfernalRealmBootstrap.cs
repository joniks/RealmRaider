    ...
    public static class InfernalRealmBootstrap
    {
        ...
        static void Build()
        {
            ...
            PrototypeArenaBoundaryBuilder.BuildRectangle(root.transform, Vector3.zero, new Vector2(14, 68), 0,
                PrototypeArenaBoundaryStyle.InfernalBasalt, 1.35f, 1, 5, .4f);
            ...
            var invader = PrototypeRuntimeFactory.CreateEntity(PrototypeCharacterRoster.BloodKnightId, "Invading Blood Knight", new Vector3(0, 1, -30), invaderStats, new Color(.7f, .04f, .03f), false, Vector3.one * .95f, new[] { PrototypeRuntimeFactory.Ability("Basic Slash", AbilityKind.Melee, 22, 2.3f, .9f, .18f, .9f), PrototypeRuntimeFactory.Ability("Blood Rush", AbilityKind.Dash, 25, 1.8f, 3, .15f, .9f, 6), PrototypeRuntimeFactory.Ability("Heavy Cleave", AbilityKind.Area, 35, 1.8f, 2.8f, .65f) }, false);
            var bruteStats = CombatStats.Ent; bruteStats.MaxHealth = 380; bruteStats.AttackDamage = 38; bruteStats.MoveSpeed = 2.8f;
            var brute = PrototypeRuntimeFactory.CreateEntity(PrototypeCharacterRoster.InfernalBruteId, "Infernal Brute", new Vector3(0, 1.5f, 10), bruteStats, new Color(.3f, .08f, .045f), true, Vector3.one * 1.45f, new[] { PrototypeRuntimeFactory.Ability("Smash", AbilityKind.Melee, 40, 2.7f, 1.3f, .5f, .9f), PrototypeRuntimeFactory.Ability("Charge", AbilityKind.Dash, 28, 1.8f, 3, .22f, .9f, 5), PrototypeRuntimeFactory.Ability("Ground Slam", AbilityKind.Area, 46, 1, 4, .8f) }, true);
            var houndStats = new CombatStats { MaxHealth = 60, AttackDamage = 12, AttackSpeed = 1.5f, MoveSpeed = 7.2f, Armor = 2, AbilityPower = 5 };
            var houndOne = PrototypeRuntimeFactory.CreateEntity(PrototypeCharacterRoster.HellhoundId, "Hellhound A", new Vector3(-3.5f, .7f, -2), houndStats, new Color(.38f, .07f, .025f), false, Vector3.one * .7f, new[] { PrototypeRuntimeFactory.Ability("Leap", AbilityKind.Melee, 12, 2.4f, 1, .14f, .9f) }, false); var houndTwo = PrototypeRuntimeFactory.CreateEntity(PrototypeCharacterRoster.HellhoundId, "Hellhound B", new Vector3(3.5f, .7f, 3), houndStats, new Color(.48f, .1f, .025f), false, Vector3.one * .7f, new[] { PrototypeRuntimeFactory.Ability("Leap", AbilityKind.Melee, 12, 2.4f, 1, .14f, .9f) }, false);
            ...
            var flameObject = GameObject.CreatePrimitive(PrimitiveType.Cylinder); flameObject.name = "Flame Trap"; flameObject.transform.position = new Vector3(0, .1f, -7); flameObject.transform.localScale = new Vector3(2.7f, .1f, 2.7f); RealmLandmarkPresentation.Build(flameObject.transform, RealmLandmarkRecipe.InfernalFlameTrap); var flame = flameObject.AddComponent<FlameTrap>(); flame.Initialize(invader);
